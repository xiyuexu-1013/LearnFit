...
export function buildSessionCsv({ samples = [] }) {
  const columns = ['elapsed_seconds', 'time', 'rhythm_score', 'signal_available'];
...
export function buildSessionReportHtml({ createdAt, task, durationSeconds, report, recommendations = [], samples = [], example = false }) {
  ...
  const metrics = [
    ['Average rhythm score', report.averageScore === null ? 'Not enough data' : `${report.averageScore} / 100`],
    ['Study time', clock(durationSeconds)],
    ['Suggested check-in', report.suggested || 'Choose your pace'],
    ['Most stable period', stable],
    ['Rhythm shift', shift],
    ['Data quality', report.quality === null ? 'No study data' : `${report.quality}% usable`],
  ];
  ...
  <header><span class="tag">LEARNFIT · PERSONAL SESSION REPORT</span><h1>Your study rhythm.</h1><p class="meta">${escapeHtml(created.toLocaleString('en'))}${task ? ` · ${escapeHtml(task)}` : ''}</p></header>
  ${example ? '<p class="notice">Example report: this file contains illustrative data, not personal measurements.</p>' : ''}
  <section class="metrics">${cards}</section>
  <section class="panel"><span class="tag">THE WHOLE SESSION</span><h2>Rhythm signal trace</h2>${chartSvg(samples, durationSeconds)}<p class="fine">Gaps mean tracking was unavailable. The score summarizes eye behavior relative to a personal baseline; it is not a percentage of attention.</p></section>

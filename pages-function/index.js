...
async function recordUsage(request, env) {
  ...
  await env.DB.prepare(`INSERT INTO daily_usage (day, source, event_type, event_count, duration_seconds, duration_count)
    VALUES (date('now'), ?, ?, 1, ?, ?)
    ON CONFLICT(day, source, event_type) DO UPDATE SET
      event_count=event_count+1,
      duration_seconds=duration_seconds+excluded.duration_seconds,
      duration_count=duration_count+excluded.duration_count`)
    .bind(usage.source, usage.eventType, usage.durationSeconds, usage.eventType === 'completed' ? 1 : 0).run();
...
async function summary(env) {
  ...
  const totalDurationSeconds = Number(usage?.duration_seconds || 0);
  const durationSessionCount = Number(usage?.duration_count || 0);
  return {
